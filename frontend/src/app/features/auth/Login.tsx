...
export function Login() {
  ...
  const features = [
    { icon: BookOpen, text: t('auth.login.features.examManagement'), color: "text-blue-400" },
    { icon: Users, text: t('auth.login.features.studentTracking'), color: "text-green-400" },
    { icon: Award, text: t('auth.login.features.autoGrading'), color: "text-purple-400" },
    { icon: TrendingUp, text: t('auth.login.features.analytics'), color: "text-orange-400" },
  ];

  const stats = [
    { value: "50K+", label: t('auth.login.stats.teachers'), icon: Users },
    { value: "500K+", label: t('auth.login.stats.students'), icon: Globe },
    { value: "1M+", label: t('auth.login.stats.tests'), icon: BookOpen },
    { value: "99%", label: t('auth.login.stats.satisfaction'), icon: Award },
  ];
          ...
          <div className="hidden lg:block space-y-8 text-white">
            <div className="space-y-6">
              <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full border border-white/20">
                ...
                <span className="font-bold text-lg">{t('auth.login.brandName')}</span>
              </div>
              
              <h1 className="text-5xl font-bold leading-tight">
                <span className="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">
                  {t('auth.login.teacherDashboard')}
                </span>
                <br />
                <span className="text-white">{t('auth.login.platformTitle')}</span>
                <br />
                <span className="text-white">{t('auth.login.platformSubtitle')}</span>
              </h1>
          ...
          <div className="w-full">
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl p-8 lg:p-12">
              ...
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-white mb-3">{t('auth.login.title')}</h2>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                <button className="flex items-center justify-center gap-3 px-4 py-3 bg-white/10 hover:bg-white/20 backdrop-blur-sm rounded-xl border border-white/20 transition-all duration-300 group">
                  <svg className="w-5 h-5" viewBox="0 0 24 24">
                    ...
                    <path fill="currentColor" className="text-blue-400" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                    <path fill="currentColor" className="text-yellow-400" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                    <path fill="currentColor" className="text-green-400" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                  </svg>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-white">{t('auth.login.email')}</label>
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-white">{t('auth.login.password')}</label>
                  <div className="relative">
