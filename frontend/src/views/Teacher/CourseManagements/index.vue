    ...
    <div class="course-management">
        ...
        <div class="action-section">
            ...
            <button class="btn-secondary" @click="isImportModalOpen = true">
                <i class="bi bi-file-earmark-arrow-up"></i> Import Course
            </button>
            <button class="btn-secondary" @click="isExportModalOpen = true">
                <i class="bi bi-download"></i> Export Course
            </button>
        </div>

        <ImportCourse :isOpen="isImportModalOpen" :categories="categories" @close="isImportModalOpen = false"
            @imported="fetchCourses" />

        <ExportModal :isOpen="isExportModalOpen" :data="courseList" @close="isExportModalOpen = false" />

        <!-- Create Course Modal -->
        <CreateCourse :isModalOpen="isModalOpen" :categories="categories" :form="form" :submitting="submitting"
            @close-modal="closeModal" @create-course="handleCreateNewCourse" />

        <!-- Course List -->
        <ListCourse :courseList="courseList" :loading="loading" :categories="categories" @fetch-courses="fetchCourses"
            @edit-course="openEditModal" />
        <!-- Course Edit -->
        <EditCourse :is-open="isEditModalOpen" :course-data="selectedCourse" :categories="categories"
            @close="isEditModalOpen = false" @updated="fetchCourses" />
    </div>
</template>
...
import ListCourse from './ListCourse/ListCourse.vue';
import CreateCourse from './CreateCourse/CreateCourse.vue';
import EditCourse from './EditCourse/EditCourse.vue';
import ImportCourse from './ImportData/ImportCourse.vue';
