...
function getFormatMeta(format?: string) {
  ...
  if (key === 'MINI_MOCK') {
    return { label: 'Mini Mock', icon: Layers, color: '#0EA5E9', bg: '#E0F2FE' };
  }
  if (key === 'DIAGNOSTIC') {
    return { label: 'Diagnostic', icon: Target, color: '#F97316', bg: '#FFEDD5' };
  }
  return { label: 'Full 4 Skills', icon: FileCheck, color: '#7C3AED', bg: '#EDE9FE' };
}
      ...
      <div className="sticky top-[73px] z-30 bg-white/80 backdrop-blur-xl rounded-2xl p-4 flex flex-col xl:flex-row xl:items-center justify-between gap-4"
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            style={{ fontSize: 14, fontWeight: 600, background: "#F9FAFB", border: "1.5px solid #F0EEFF", color: "#374151" }}
          >
            <option value="all">Dạng đề</option>
              ...
              return (
                <div key={test.assignment_id} className="relative bg-white rounded-3xl p-6 transition-all duration-300 group hover:-translate-y-1"
                   ...
                   <div className="flex gap-2 mb-6">
                     <span className="px-2.5 py-1 rounded-lg text-xs font-bold" style={{ background: formatMeta.bg, color: color }}>
                       {formatMeta.label}
                      </span>
