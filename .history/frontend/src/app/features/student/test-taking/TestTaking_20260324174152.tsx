        ...
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: "#1F1344" }}>
              {unanswered > 0 ? "Con cau chua tra loi" : "San sang nop bai"}
            </h2>
            <p className="mt-2" style={{ fontSize: 14, color: "#6B7280" }}>
              {unanswered > 0
                ? `Ban con ${unanswered} cau chua tra loi.`
                : `Ban da tra loi ${answered}/${total} cau.`}
            </p>
          </div>
...
export function TestTaking() {
  ...
  const startSpeakingRecording = async (question: any) => {
    ...
    try {
      ...
      alert("Khong the truy cap microphone. Vui long cap quyen va thu lai.");
    }
  };
            ...
            {activeSkill === "listening" && (
              <section className="space-y-5">
                ...
                <div className="rounded-md border border-violet-300 bg-violet-700 text-white p-2 flex items-center gap-3">
                  ...
                  <div className="text-sm font-semibold">Audio (chi nghe 1 lan)</div>
                  ...
                  {listeningPlayedSections[activeSectionKey] && (
                    <span className="text-[11px] px-2 py-0.5 rounded bg-white/20">Da nghe xong</span>
                  )}
                  <button
            ...
            <div className="flex flex-wrap gap-1">
              {sections.map((section) => {
                ...
                return (
                  ...
                  >
                    Part {section.part} ({answeredSection}/{section.count})
                  </button>
                );
              })}
            </div>
